  ...
  static async association(req: Request, res: Response) {
    ...
    try {
      ...
      const result = await withMongoTransaction(async (session) => {
        ...
        const profitToday = await TransactionModel.aggregate([
          {
            $match: {
        ...
        const overallRevenue = await TransactionModel.aggregate([
          ...
          {
            $match: {
              "beneficiaryDetails.userId": userId,
            },
          },
          {
            $group: {
        ...
        const revenueChangeOverTime = await TransactionModel.aggregate([
          { $match: matchConditions },
          {
            $group: {
              _id: {
        ...
        return {
          ...
          profitToday: profitToday[0]?.totalProfit || 0,
          overallRevenue: overallRevenue[0]?.totalRevenue || 0,
          revenueChangeOverTime: revenueChangeOverTime,
